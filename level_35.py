from collections import deque # we are using this so we can use the rotate method 
...
def circular_check(num: int):
    ...
    for j in range(1, len(num)): # we need to rotate as many times as the number is long, we don't need an extra + 1 at the end because we know it's original value is already prime
        ...
        copy_num = int(''.join(num)) # create a copy of the current rotation and convert it back to an integer
        if not prime_tester(copy_num): # check if it is prime, if not we stop testing
            return False
    return True
...
print(len(circular_nums)) # final answer 